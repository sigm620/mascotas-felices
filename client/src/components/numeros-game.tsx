...
function generarPregunta() {
  ...
  if (tipo === 'contar') {
    const cantidad = Math.floor(Math.random() * 5) + 1; // 1-5
    const respuesta = cantidad;
    const opciones = generarOpciones(respuesta, 1, 6);
    return { tipo, emoji, cantidad, respuesta, opciones, emoji2: null, cantidad2: 0 };
  } else {
    const a = Math.floor(Math.random() * 3) + 1; // 1-3
    const b = Math.floor(Math.random() * 3) + 1; // 1-3
    const emoji2 = EMOJIS_CONTEO[Math.floor(Math.random() * EMOJIS_CONTEO.length)];
    const respuesta = a + b;
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-300 via-cyan-200 to-teal-300 p-4 flex items-center justify-center">
      <Card className="w-full max-w-sm p-6 shadow-2xl">
        ...
        <div className="bg-blue-50 rounded-2xl p-4 mb-4 text-center min-h-32 flex flex-col items-center justify-center">
          {p.tipo === 'contar' ? (
            <>
              ...
              <p className="text-sm font-bold text-gray-600">¿Cuántos hay?</p>
            </>
          ) : (
            <>
              <div className="flex items-center gap-2 text-3xl mb-2 flex-wrap justify-center">
                ...
                <span className="flex gap-1">
                  {Array.from({ length: p.cantidad2 }, (_, i) => <span key={i}>{p.emoji2}</span>)}
                </span>
              </div>
              <p className="text-sm font-bold text-gray-600">¿Cuántos en total?</p>
            </>
          )}
        </div>
        ...
        <div className="mt-4 bg-gray-200 rounded-full h-2">
          <div
            className={`h-2 rounded-full transition-all ${timeLeft <= 5 ? 'bg-red-400' : 'bg-blue-400'}`}
